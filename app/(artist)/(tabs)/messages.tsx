...
import { listArtistThreads } from "../../../src/features/artist-messages/data";
import type { ThreadSummary } from "../../../src/features/artist-messages/types";
...
export default function Messages() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const rows = await listArtistThreads();
      setThreads(rows);
      setStatus("ready");
    } catch {
      setStatus("error");
    ...
    return (
      <Screen padded={false}>
        <View className="flex-1 items-center justify-center px-8">
          ...
          >
            Every client conversation in one place — references, quotes and
            aftercare, kept tidy per booking.
          </Text>
          <Button
            label="Sign in"
            variant="primary"
    ...
    <Screen padded={false}>
      <FlatList
        ...
        renderItem={({ item }) => (
          <ThreadRow
            thread={item}
            onPress={() => router.push(`/(artist)/thread/${item.id}`)}
          />
        )}
      />
    </Screen>
    ...
    >
      ...
      <View className="flex-1">
        <View className="flex-row items-center gap-2">
          ...
          <Text
            variant="caption"
            className={unread ? "text-gold-300" : "text-bone-500"}
          >
            {timeLabel}
          </Text>
        </View>
