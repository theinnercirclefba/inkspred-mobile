...
import {
  ...
  timeLabel,
  dayLabel,
} from "../../../src/features/messages/data";
import type {
  OtherParty,
  QuoteView,
  ThreadMessage,
...
function buildRows(messages: ThreadMessage[]): Row[] {
  ...
  for (const m of messages) {
    if (m.dayLabel !== lastDay) {
      rows.push({ kind: "divider", id: `divider-${m.dayLabel}-${m.id}`, label: m.dayLabel });
      lastDay = m.dayLabel;
    }
...
export default function ThreadScreen() {
    ...
    return (
      <View className="flex-1 bg-ink-950">
        <SafeAreaView edges={["top"]}>
          <ThreadHeader other={null} onBack={() => router.back()} onOpenProfile={null} />
        </SafeAreaView>
  ...
  const openProfile =
    other?.role === "artist" && other.handle
          ...
          ListEmptyComponent={
            <View className="items-center px-8 pt-16">
              <Text variant="body" className="text-center text-bone-500">
                Say hello — start the conversation about your piece.
              </Text>
            </View>
          }
          renderItem={({ item }) =>
...
function Composer({
  ...
  return (
    <SafeAreaView edges={["bottom"]} className="border-t border-ink-800 bg-ink-900">
      {error ? (
